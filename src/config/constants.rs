...
pub const PROTOCOL_SESSION_CACHE_NAME: &str = "protocol_sessions.json";
...
pub static PROTOCOL_SESSION_CACHE_PATH: LazyLock<PathBuf> = LazyLock::new(|| {
    CONFIG_PATH
        .parent()
        .map(|path| path.join(PROTOCOL_SESSION_CACHE_NAME))
        .unwrap_or_else(|| PathBuf::from(PROTOCOL_SESSION_CACHE_NAME))
});
